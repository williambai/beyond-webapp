...
var _login = function(account, done){
	models.CbssAccount.login(account, function(err){
		//** 登录失败，重新登录，直至成功
		if(err){
			logger.warn('登录失败：'+ JSON.stringify(err));
			logger.warn('5秒后尝试重新登录：' + JSON.stringify(account));
			setTimeout(function(){
				_login(account,done);
			},5000);
			return;
		...
		//** 更新accountsEnable
		var newAccountEnable = [];
		accountsEnable.forEach(function(acc){
			if(acc.id != account.id) newAccountEnable.push(acc);
		});
		newAccountEnable.push(account);
		accountsEnable = newAccountEnable;
		done && done();
	});
};

var findAllAccountAndLogin = function(next){
	//** 找到所有的有效accounts
	models.CbssAccount.findByStatus({
		status: '有效',
...
var refreshCookieJob = function(accountsEnable){
		async.eachSeries(accountsEnable, function(account,callback){
			...
			setTimeout(function(){
				...
				refreshCookieJob(accountsEnable);
			},refreshPeroid);
		});
	};

//** 定时处理 4G 订单处理,每过7秒钟检查一次订单
var processOrderJob = function(accountsEnable){
		models.Order.process4G(accountsEnable, function(err,result) {
			if (err)logger.error(err);
			result = result || {};
			if(result.logout){
				logger.info('call 4G Order peroid job fail, bacause of account logout.');
				// setTimeout(function(){
				// 	_login(result.account || {});
				// },5000);
			}else{
...
//** 启动
var startService = function(done){
		findAllAccountAndLogin(function(err){
