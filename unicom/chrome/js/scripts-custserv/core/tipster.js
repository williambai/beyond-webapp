...
{
 for (var t in TipObj.list) if(TipObj.list[t].track)TipObj.list[t].track(evt);
...
function explainToTips(tipType,tipWidth,_doc)
{
    if(!_doc)
		_doc=document;
    var tags = isDOM ? _doc.getElementsByTagName('*') : [];
    for (var i = 0; i < tags.length; i++)
    {
        if (tags[i].explain)
        {
            tags[i].onmouseover = new Function(tipType+'.newTip("tagTip' + i + '", 10, 10, '+tipWidth+'  ,' +
                tags[i].explain.inspect(true) + ')');
            tags[i].onmouseout = new Function(tipType+'.hide()');
        }   
    }       
};

var docTip = new TipObj('docTip');
docTip.template = '<table bgcolor="#003366" cellpadding="1" cellspacing="0" width="%2%" border="    ">' +
        '<tr><td><table bgcolor="#6699CC" cellpadding="3" cellspacing="0" width="100%" border="0">' +
        '<tr><td style="font:10px Arial,Helvetica;color: white">%3%</td></tr></table></td></tr></table>';
var staticTip = new TipObj('staticTip');
staticTip.template = '<table bgcolor="#000000" cellpadding="0" cellspacing="0" width="%2%" border="0">' +
  '<tr><td><table cellpadding="3" cellspacing="1" width="100%" border="0">' +
  '<tr><td bgcolor="#6699CC"     height="18" class="tipClass">%3%</td></tr>' +
  '</table></td></tr></table>';
staticTip.tipStick = 0;
var stickyTip = new TipObj('stickyTip');
stickyTip.template = '<table bgcolor="#000000" cellpadding="1" cellspacing="0" width="%2%" border="0">' +
