...
function startFormat(str) {
	...
	if (js_source && js_source.charAt(0) === '<') {
		js_source = style_html(
				js_source, tabsize_1, tabchar, 80);
	} else {
		js_source = js_beautify(
				js_source, tabsize_1, tabchar);
	}
	js_source = js_source.replace(new RegExp('td\\|\\|','g'),'td ||');
	js_source = js_source.replace(new RegExp('tr\\|\\|','g'),'tr ||');
	js_source = js_source.replace(new RegExp('tdcolspan','g'),'td colspan');
	return js_source;
}
/**
 * 压缩html传
 * @param input
 * @param base64（该参数用户压缩，不要传）
 * @returns
 * @author zhangyangshuo
 */
function packJs(input,base64) {
	...
	if (base64) {
		...
		output = output.replace(new RegExp('td\\|\\|','g'),'td ||');
		output = output.replace(new RegExp('tr\\|\\|','g'),'tr ||');
		output = output.replace(new RegExp('tdcolspan','g'),'td colspan');
...
function copyFromClipboard(element){
	if(window.clipboardData){
