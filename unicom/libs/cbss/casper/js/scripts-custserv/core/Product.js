...
function showProductInfoByArea(showArea,node)
{
	...
	$A(document.getElementsByName('_productinfos')).each(function(prod) {
		 //qc:51882 begin
		  if(prod.checked && (prod.productMode == '60'||( prod.productMode=='00'  && tradeTypeCode==796))&&prod.parentArea == productArea) {
		//qc:51882 end	
			  //循环累积基础产品
			  queryPackageInfo(prod.productId);
...
function changePtype(){
	...
	var prodInfo={};
	$A(document.getElementsByName('_productinfos')).each(function(prod) {
	  if(prod.checked && prod.productMode == '00'&&prod.parentArea == productArea) {
		    //循环累积基础产品
		    prodInfo=prod;
		 }
     }); 
...
function dealAfterGetPrePagProInfo(node){
	...
	$A(document.getElementsByName('_productinfos')).each(function(prod) {
		if(prod.parentArea == productArea) { 
		        if(prod.productMode=="00"){
		  			$("_p"+prod.productId).checked = true;
		  			//queryPackageInfo(prod.productId);
		  			onProductClick(prod.productId,true);
		        }
		        if(prod.productMode=="50"){
		        	$("_p"+prod.productId).checked = true;
		        } 
		 }
	});
...
function checkedMustProduct(){
	if(getRightCode() == "csCreateWileBossYUserTrade" ||  getRightCode()=='csCreateWileProvYUserTrade' || getRightCode() == "csCreateWileBossYUserTrade4G"){
		...
		$A(document.getElementsByName('_productinfos')).each(function(prod) {
			if(prod.productMode == '00'&&prod.parentArea == productArea) {
				  //循环累积基础产品
				  productSelf=prod;
			      productCountSelf++
			}
...
function queryPackageInfo(productId){   
	...
	if($("p"+productId).first.toUpperCase() == 'FALSE') {
		$("p"+productId).toggle();	 
        closeOpen($("p"+productId),$("closeopen"+productId));   
		return;
	}
...
function showPackageInfo(node){
	...
	$A(document.getElementsByName('_productinfos')).each(function(prod) {
	//qc:51882 begin
		  if(prod.checked && (prod.productMode == '60'||(tradeTypeCode==796||tradeTypeCode==196))&&prod.parentArea == productArea) {
	//qc:51882 end
			  //黑莓产品默认展开
			  
				$A($('p'+prod.productId).all).each(function (s) {
					if(s.tagName.toUpperCase() == 'INPUT' && s.type.toUpperCase() == 'CHECKBOX'
						&& s._thisType != 'undefined' && s._thisType.toUpperCase() == 'PACKAGE'
						&& s.checked) {
						queryElementByPkgId(s.packageId,prod.productId);
					}
				});
			 }
	});
...
function queryElementByPkgId(packageId, productId){
	...
	if($("p"+productId+"k"+packageId).first.toUpperCase() == 'FALSE') {
 ...
 * @author zhoush
 */
function onProductClick(productId, checked, bubble) { 
	//tfs:208234 Begin
	...
	$A(document.getElementsByName('_productinfos')).each(function(prod) {
		  if(prod.productMode == '50'&&prod.parentArea == "deviceProdutArea") {
			  	//查看营销活动中是否存在多个
			  	if(productId != prod.productId && $('_p'+prod.productId).checked) {
			  		$('_p'+prod.productId).checked = false;
			  		...
			  		$A($('p'+prod.productId).all).each(function (s) {
			  			if(s.tagName.toUpperCase() == 'INPUT' && s.type.toUpperCase() == 'CHECKBOX'
			  				&& s._thisType != 'undefined' && s._thisType.toUpperCase() == 'PACKAGE') {
			  					//触发包的onclick事件
			  					s.checked = false;
			  					onPackageClick(prod.productId, s.packageId, false, false);
			  			}
			  		});
			  	}
			 }
	...
	//tfs:177905 begin 
 	if(checked && $('_p'+productId).modifyTag == '0' && $('_p'+productId).productMode == '00' && $('_p'+productId).netTypeCode == '40') {
	...
	//新增产选中时自动展开
	if(checked && $('_p'+productId).modifyTag == '0' && $('_p'+productId).needExp == '1') {
	...
	$A($('p'+productId).all).each(function (s) {
		if(s.tagName.toUpperCase() == 'INPUT' && s.type.toUpperCase() == 'CHECKBOX'
			&& s._thisType != 'undefined' && s._thisType.toUpperCase() == 'PACKAGE') {
				...
				if(checked) {
					if(s.forceTag == '1' || (s.defaultTag == '1' && curProductId != productId ) || s.modifyTag == '9') s.checked = checked;

					if(s.forceTag == '1') s.disabled = true;
				}
				else {
	...
	if(typeof drawContractInfoArea != 'undefined' && drawContractInfoArea instanceof Function && typeof sfUserInfo != 'undefined'  &&  null != sfUserInfo
		...
		$A(document.getElementsByName('_productinfos')).each(function(prod) {
			   if (prod.netTypeCode == '50' && $('_p'+prod.productId).checked && prod.productMode == '00') {
					   $("contractInfoArea").innerHTML ="";
					   //tfs:249355  只有点击主产品的时候才会重新刷新原合约信息,当点击促销类的附加产品等的时候不会重新刷新该区域,否则可能导致赠款规则不正确
		               drawContractInfoArea(prod);
		               $("contractInfoArea").pid = prod.productId;
		       }
		});
	}
	//tfs：88873 end
 ...
 * @author zhoush
 */
function onPackageClickFromEle(productId, packageId, checked, bubble) {
	//if(getRightCode()=='csCreateWileProvYUserTrade')
	//	drawTerminalCtrlByPro(productId);
	if(getRightCode() == "csCreateWileGrpUserTrade" || getRightCode() == "csCreateWilePerUserTrade" || getRightCode() == "csCreateWileGrpUserTrade4G" || getRightCode() == "csCreateWilePerUserTrade4G" || getRightCode() == "csCreateWilePerUserJoinWO"){
	//  后付费无线上网卡 参加活动查询活动费用
	...
	if(bubble) {
		...
		//元素被取消
			...
			$A($('p'+productId+'k'+packageId).all).each(function (s) {
				if(s.tagName.toUpperCase() == 'INPUT' && s.type.toUpperCase() == 'CHECKBOX'
					&& s._thisType != 'undefined' && s._thisType.toUpperCase() == 'ELEMENT'
					&& s.checked) {
					allCancel = false;
				}
			});
			...
			if(allCancel && $('_p'+productId+'k'+packageId).forceTag != '1')
	...
	//新增包选中时自动展开
	if(checked && ($('_p'+productId+'k'+packageId).modifyTag == '0'||$('_p'+productId+'k'+packageId).modifyTag=='9') && $('_p'+productId+'k'+packageId).needExp == '1') {  //modify by tz@2009-11-8 02:58下午 增加 modify_tag='9'
		queryElementByPkgId(packageId,productId);
	...
	$A($('p'+productId+'k'+packageId).all).each(function (s) {
		if(s.tagName.toUpperCase() == 'INPUT' && s.type.toUpperCase() == 'CHECKBOX'
			&& s._thisType != 'undefined' && s._thisType.toUpperCase() == 'ELEMENT') {
				...
				if(checked) {
					if(s.forceTag == '1' || (s.defaultTag == '1'&&curProductId != productId) || s.modifyTag == '9') 
					//qc:3803 用于固网产品及属性变更页面 add by xiexc begin
					{
					  ...
					  //qc 98337 start
                      else if(s.endDate == Cs.util.Utility.getLastDay(Cs.ctrl.Trade.getSysDate())&& $('_p'+productId).modifyTag =='0'){
				...
				//触发元素的onclick事件
				onElementClick(productId, packageId, s.value,s.elementTypeCode,s.checked, false);
		}
	});	
	//触发自下而上事件
 ...
 * @author zhoush
 */
function onElementClick(productId, packageId, elementId,elementType, checked, bubble, passed, isEleClick) {
    var eId = "_p"+productId+"k"+packageId+"e"+elementId+"T"+elementType;
		    ...
		    && !s.checked) 
		{
			relyEles.each(function(xe){
					...
					||(send<=oend&&send>ostart)))
				{
					s.checked = true;
					var eId = s.id;
					onElementClick(eId.split("k")[0].substr(2), s.packageId, s.elementId,eId.split("T")[1], true, true);
					return;
				}
			});    		    
		}
...
function checkOperator(productId, packageId) {
    ...
    if (pdt.maxNumber!="-1"||pdt.minNumber!="-1")
        needCheckPdt=true;
    if ($('p'+productId+'k'+packageId).first.toUpperCase()=="FALSE"||(pkg.maxNumber!="-1"||pkg.minNumber!="-1"))
        needCheckPkg=true;
		...
		if(prod.tagName.toUpperCase() == 'INPUT' && prod.type.toUpperCase() == 'CHECKBOX'
			...
			$A($('p'+prod.productId).all).each(function(elem) {
				if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
					...
					var elemDate = compElemDate($('_p'+elem.productId), elem); 
...
function compSelectedProdDate(paramProd) {
	...
	$A($(productArea).all).each(function(prod) {
		if(prod.tagName.toUpperCase() == 'INPUT' && prod.type.toUpperCase() == 'CHECKBOX'
			&& prod._thisType != 'undefined' && prod._thisType.toUpperCase() == 'PRODUCT' && prod.productId == paramProd) {
			
			selProdStartDate = 	prod._startDate;
		    selProdEndDate =  prod._endDate;
			//重算元素生效、失效时间
			$A($('p'+prod.productId).all).each(function(elem) {
				if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
					&& elem._thisType != 'undefined' && elem._thisType.toUpperCase() == 'ELEMENT'
	...
	$A($(productArea).all).each(function(prod) {
		if(prod.tagName.toUpperCase() == 'INPUT' && prod.type.toUpperCase() == 'CHECKBOX'
			&& prod._thisType != 'undefined' && prod._thisType.toUpperCase() == 'PRODUCT' && prod.productMode!="00" && prod.modifyTag =="0") {
				
			if (selProdStartDate != "")		
				prod._startDate = selProdStartDate;
			if (selProdEndDate != "")		
				prod._endDate = selProdEndDate;
			//重算元素生效、失效时间
			$A($('p'+prod.productId).all).each(function(elem) {
				if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
					&& elem._thisType != 'undefined' && elem._thisType.toUpperCase() == 'ELEMENT'
...
function compSvcDate(elementId) {
	...
	$A($(productArea).all).each(function(prod) {
		if(prod.tagName.toUpperCase() == 'INPUT' && prod.type.toUpperCase() == 'CHECKBOX'
			&& prod._thisType != 'undefined' && prod._thisType.toUpperCase() == 'PRODUCT' && prod.productMode=="00" && prod.checked) {
							
			//重算元素生效、失效时间
			$A($('p'+prod.productId).all).each(function(elem) {
				if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
					&& elem._thisType != 'undefined' && elem._thisType.toUpperCase() == 'ELEMENT'
					&& elem.elementTypeCode.toUpperCase() == 'S') {
				
						if(all2g.include(elem.elementId)
							&&(elem.modifyTag=="9"||(elem.modifyTag=="0"&&elem.checked)))
						{
							has2g="true";
						}
...
function geneBaseProductInfo() {
	...
	$A(document.getElementsByName('_productinfos')).each(function(prod) {
		...
		if(prod.parentArea =="productArea" && prod.productMode=='00' && prod.isDisabled == true && prod.endDate<=Cs.util.Utility.getLastDay(Cs.ctrl.Trade.getSysDate())){
    		memDisProdCount++;
    	}
		...
		if(prod.checked && prod.productMode == '00'&&prod.parentArea == productArea) {
			...
			if(tradeTypeCode == "10"||tradeTypeCode == "500"||tradeTypeCode == "503")
			{
				...
				prod._startDate=mProdStartDate;
			}
			else{
				mProdStartDate = prod._startDate;
				}
				mOldProdEndDate = Cs.util.Utility.computeDate(mProdStartDate, '6', -1);
				
			}
			mProdEndDate = prod._endDate;
			
			newProdId = prod.productId;
			newBrandCode = prod.brandCode;
			newNetCode = prod.netTypeCode;
			newPrepayTag = prod.prepayTag;
			newProductTypeCode = prod.productTypeCode;
...
function geneMemberProductInfo() {
    memProdCount = 0;
    $A(document.getElementsByName('_productinfos')).each(function(prod) {
         if(prod.checked && prod.productMode == '20' &&prod.parentArea == productArea) {
              //统计成员产品的数量
              memProdCount++;
         }
    });
...
/*是否需要展开产品、包判断*/
	$A(document.getElementsByName('_productinfos')).each(function(prod) {
		if(prod.checked&&prod.parentArea == productArea) {
			...
			$A($('p'+prod.productId).all).each(function (s) {
				if(s.tagName.toUpperCase() == 'INPUT' && s.type.toUpperCase() == 'CHECKBOX'
					...
					$A($('p'+s.productId).all).each(function (m) {
			    	    if(m.tagName.toUpperCase() == 'INPUT' && m.type.toUpperCase() == 'CHECKBOX'
					        ...
					        && m._thisType != 'undefined' && m._thisType.toUpperCase() == 'ELEMENT' && m.checked) {
					     	...
					     	var ele = $("_p"+s.productId+"k"+s.packageId+"e"+m.elementId+"T"+"D");
					     	if (ele == null)
					     		ele = $("_p"+s.productId+"k"+s.packageId+"e"+m.elementId+"T"+"S");
					     	if (ele == null)
					     		ele = $("_p"+s.productId+"k"+s.packageId+"e"+m.elementId+"T"+"X");
							if (ele == null)
					     		ele = $("_p"+s.productId+"k"+s.packageId+"e"+m.elementId+"T"+"C");
        			...
        			if(elementConut == 0  && $("p"+s.productId+"k"+s.packageId).first.toUpperCase() != 'TRUE') throw new Error('业务包：\"' + pkg.packageName + '\"至少选择1个元素，业务无法继续！');
			...
			if ($('_p'+prod.productId).maxNumber !="-1" && parseInt($('_p'+prod.productId).maxNumber,10) <packageCount)
				throw new Error('产品：\"' + prod.productName + '\"最多选择'+$('_p'+prod.productId).maxNumber+'个业务包，业务无法继续！');
						
			if ($('_p'+prod.productId).minNumber !="-1" && parseInt($('_p'+prod.productId).minNumber,10) >packageCount)
				throw new Error('产品：\"' + prod.productName + '\"最少选择'+$('_p'+prod.productId).minNumber+'个业务包，业务无法继续！');
	...
	/*是否需要展开产品、包判断*/
	$A(document.getElementsByName('_productinfos')).each(function(prod) {
		if(prod.checked&&prod.parentArea == "deviceProdutArea") {
			if((prod.needExp == '1') && $("p"+prod.productId).first.toUpperCase() == 'TRUE') {
				throw new Error('请展开产品：\"' + prod.productName + '\"进行选择操作！');
			}
		...
		if(prod.tagName.toUpperCase() == 'INPUT' && prod.type.toUpperCase() == 'CHECKBOX'
			...
			if(prod.modifyTag == '0') {
				if(prod.checked) {					
					...
					if($('p'+prod.productId).first.toUpperCase() == 'TRUE') {
						//生成产品未展开台帐
						tradeNoExp.push(geneNoExpInfo(prod.productId, '-9', '0', prod._submitStartDate, prod._submitEndDate, prod.productMode, prod.brandCode));
					}
					...
					$A($('p'+prod.productId).all).each(function(elem) {
						//包是否展开处理(处理包已选择的)
						if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
							&& elem._thisType != 'undefined' && elem._thisType.toUpperCase() == 'PACKAGE'
							&& elem.checked && $("p"+elem.productId+"k"+elem.packageId).first.toUpperCase() == 'TRUE') {
							//生成包未展开台帐
							tradeNoExp.push(geneNoExpInfo(elem.productId, elem.packageId, '0', prod._submitStartDate, prod._submitEndDate, prod.productMode, prod.brandCode));
						}
						...
						if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
							...
							//新增优惠台帐
							if(elem.elementTypeCode.toUpperCase() == 'D') {
							...
							//新增服务台帐
							else if(elem.elementTypeCode.toUpperCase() == 'S') {
							...
							//新增sp服务台帐
							else if(elem.elementTypeCode.toUpperCase() == 'X') {
							...
							//新增购机台帐
							else if(elem.elementTypeCode.toUpperCase() == 'C') {	
							...
							//新增实物台帐
							else if(elem.elementTypeCode.toUpperCase() == 'A' || elem.elementTypeCode.toUpperCase() == 'K') {
								tradeElement.push(geneElementInfo(elem, '0'));
							}
							//新增积分台帐
							else if(elem.elementTypeCode.toUpperCase() == 'I'){								
								if(score<elem.score)
			...
			//删除产品
			} else if(prod.modifyTag == '1') {
				
				//如果产品置灰并且失效时间为当月月底,则退出当前循环
				if(prod.isDisabled == true && prod.endDate<=Cs.util.Utility.getLastDay(Cs.ctrl.Trade.getSysDate())){
					return true;
				}
				...
				if($('p'+prod.productId).first.toUpperCase() == 'TRUE') {
					//生成产品未展开台帐
					tradeNoExp.push(geneNoExpInfo(prod.productId, '-9', '1', prod._submitStartDate, prod._submitEndDate, prod.productMode, prod.brandCode));
				}
				...
				$A($('p'+prod.productId).all).each(function(elem) {
					...
					if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
						&& elem._thisType != 'undefined' && elem._thisType.toUpperCase() == 'PACKAGE'
						&& $("p"+elem.productId+"k"+elem.packageId).first.toUpperCase() == 'TRUE') {
						//生成包未展开台帐
						tradeNoExp.push(geneNoExpInfo(elem.productId, elem.packageId, '1', prod._submitStartDate, prod._submitEndDate, prod.productMode, prod.brandCode));
					}					
			...
			//用户原有产品
					...
					$A($('p'+prod.productId).all).each(function(elem) {
						...
						if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
							...
							if(!elem.checked && elem.modifyTag == '9') {
								//生成包未展开台帐
								tradeNoExp.push(geneNoExpInfo(elem.productId, elem.packageId, '1', prod.startDate, mOldProdEndDate, prod.productMode, prod.brandCode));
								
								//生成用户原有包完全删除台帐信息
								tradeNoExp.push(geneDelPackInfo(elem.productId, elem.packageId, '1', prod.startDate, mOldProdEndDate, prod.productMode, prod.brandCode));
							}
							
							//新增包(modifyTag='0')，选中时生成新增未展开台帐，取消选中时不处理
							if(elem.checked && elem.modifyTag == '0') {
								//生成包未展开台帐
								tradeNoExp.push(geneNoExpInfo(elem.productId, elem.packageId, '0', mProdStartDate, prod.endDate, prod.productMode, prod.brandCode));
							}
						}
				...
				//取消
					...
					if(tradeTypeCode=='196'&&prod.productMode=='00'){
		...
		$A($("currentSaleProductArea").all).each(function(prod) {
			if(prod.tagName.toUpperCase() == 'INPUT' && prod.type.toUpperCase() == 'CHECKBOX'
				...
				if (prod.modifyTag == '1') {
					...
					//删除活动台帐
					tradeProType.push(geneProType(prod, '1', prod.productTypeCode));
					//生成属性台帐子表
					tradeSubItem = tradeSubItem.concat(geneTradeSubItemInfo('1', prod));

					$A($('p' + prod.productId).all).each(
							function(elem) {
								//包是否展开处理
								if (elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX' && elem._thisType != 'undefined'
										&& elem._thisType.toUpperCase() == 'PACKAGE' && $("p" + elem.productId + "k" + elem.packageId).first.toUpperCase() == 'TRUE') {
									//生成包未展开台帐
									tradeNoExp.push(geneNoExpInfo(elem.productId, elem.packageId, '1', prod._submitStartDate, prod._submitEndDate, prod.productMode,
											prod.brandCode));
								}

								if (elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX' && elem._thisType != 'undefined'
										&& elem._thisType.toUpperCase() == 'ELEMENT') {
				...
				if(prod.modifyTag == '0') {
					if(prod.checked) {					
						...
						if (myDeviceAllInfo !=""){
							...
							var activeStartDate = prod._startDate > mProdStartDate ? prod._startDate : mProdStartDate;
					   ...
					   //qc:95752 end
						...
						if($('p'+prod.productId).first.toUpperCase() == 'TRUE') {
							//生成产品未展开台帐
							tradeNoExp.push(geneNoExpInfo(prod.productId, '-9', '0', prod._submitStartDate, prod._endDate, prod.productMode, prod.brandCode));
						}
						...
						$A($('p'+prod.productId).all).each(function(elem) {
							...
							if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
								...
								&& elem.checked && $("p"+elem.productId+"k"+elem.packageId).first.toUpperCase() == 'TRUE') {
								//生成包未展开台帐
								tradeNoExp.push(geneNoExpInfo(elem.productId, elem.packageId, '0', prod._submitStartDate, prod._endDate, prod.productMode, prod.brandCode));
							}
							...
							if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
									...
									if (myDeviceAllInfo !=""){
										...
										if (!isGeneTradeId){
										    ...
										    //if($('pagecontext').provinceId=='0076'&&getRightCode() == "csCreateUserTrade"){    //qc:95934
										        tradeSubItem=tradeSubItem.concat(geneSubItem(elem,discntInfo,"3",prod._submitStartDate, prod._endDate));
										   // }else{
								...
								else if(elem.elementTypeCode.toUpperCase() == 'K') {
									tradeElement.push(geneElementInfo(elem, '0'));
								}
								else if(elem.elementTypeCode.toUpperCase() == 'A') {
									isHasElement = true;
									tradeElement.push(geneElementInfo(elem, '0'));
								...
								else if(elem.elementTypeCode.toUpperCase() == 'B') {
									isHasElement = true;
									if(typeof elem.itemObjNew=="undefined"||typeof elem.itemObjNew.mytradeinfo=="undefined")									
...
//			throw new Error('自备机场景场景更新异常，请关掉页面重新受理！');
	...
	$A($(productArea).all).each(function(prod) {
		if(prod.tagName.toUpperCase() == 'INPUT' && prod.type.toUpperCase() == 'CHECKBOX'
			&& prod._thisType != 'undefined' && prod._thisType.toUpperCase() == 'PRODUCT' 
			&& prod.checked && prod.netTypeCode == "40" && prod.productMode == "01" && prod.modifyTag == '9'
			&& prod.endDate < Cs.util.Utility.computeDate(Cs.ctrl.Trade.getSysDate(), "3", 1).substring(0,10)) {
					var hasSvcAdd = false;
					...
					$A($('p'+prod.productId).all).each(function(elem) {
						...
						if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
							...
							if(elem.checked && elem.modifyTag == '0') {
								...
								else if(elem.elementTypeCode.toUpperCase() == 'S' || elem.elementTypeCode.toUpperCase() == 'X') {
									hasSvcAdd = true;
								}																
							}
						}
					...
					if(hasSvcAdd && hasDiscntAdd){
						for(var i=0; i<tradeProduct.length; i++){
							if(tradeProduct[i].PRODUCT_ID == prod.productId){
								hasProduct = true;
							}
						}
					}
					if(hasSvcAdd && hasDiscntAdd && !hasProduct){
						...
						var itpvProdType = geneProType(prod, '2', prod.productTypeCode);
						...
						for(var i=0; i<tradeSvc.length; i++){
							if(tradeSvc[i].PRODUCT_ID == prod.productId){
						...
						for(var i=0; i<tradeDiscnt.length; i++){
							if(tradeDiscnt[i]!=undefined && tradeDiscnt[i].PRODUCT_ID == prod.productId){
						...
						for(var i=0; i<tradeSp.length; i++){
							if(tradeSp[i].PRODUCT_ID == prod.productId){
...
function geneProdInfo(prod, modifyTag) {
	...
	if(modifyTag == '0') {
		...
		startDate = prod._startDate > mProdStartDate ? prod._startDate : mProdStartDate;
		endDate = prod._endDate;		
	} else if(modifyTag == '1') {
	...
	params += 'PRODUCT_ID='+prod.productId + '&PRODUCT_MODE='+prod.productMode + '&START_DATE='+startDate + '&END_DATE='+endDate;
...
function geneProType(prod, modifyTag,productTypeCodeIn) {
	...
	{
		if(typeof prod.productTypeCode != "undefined")
		{
			newProductTypeCode = prod.productTypeCode;
		}
		else
...
function chkLTE4SerialNumber(chkSerialNumber){
	...
	$A($(productArea).all).each(function(prod) {
		if(prod.tagName.toUpperCase() == 'INPUT' && prod.type.toUpperCase() == 'CHECKBOX'
						&& prod._thisType != 'undefined' && prod._thisType.toUpperCase() == 'PRODUCT' && prod.checked)
		{
			$A($('p'+prod.productId).all).each(function(elem) 
			{			
				if(elem.tagName.toUpperCase() == 'INPUT' && elem.type.toUpperCase() == 'CHECKBOX'
							&& elem._thisType != 'undefined' && elem._thisType.toUpperCase() == 'ELEMENT'
							&& elem.checked && elem.elementTypeCode.toUpperCase() == 'S') 
				{
					if ($("ISNEED_LET_CHECK") && $("ISNEED_LET_CHECK").value =="1"){ //总开关
