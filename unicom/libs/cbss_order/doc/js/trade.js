...
function afterTradeSubmitOk(node){		
		...
		&&Cs.ctrl.Trade._tradeInfo.TRADE_ITEM.SUB_TYPE=='5')
	{
		...
		if(feenum ==0)
			$("feeArea").onshow=redirectTo('personalserv.dealtradefee.DealTradeFee','init', "&TRADE_TYPE_CODE=tradeType&param="+encodeURIComponent(Object.toJSON(params))+"&fee="+"&noBack="+noBack, 'feeframe');								
		else 
		{
			if(Cs.ctrl.Trade._tradeInfo && Cs.ctrl.Trade._tradeInfo.ITEM_INFOA && Cs.ctrl.Trade._tradeInfo.ITEM_INFOA.ORDER_TYPE=='2')
			{
				...
				$("feeArea").onshow=redirectTo('personalserv.dealtradefee.DealTradeFee','init', "&TRADE_TYPE_CODE=tradeType&param="+encodeURIComponent(Object.toJSON(params))+"&fee="+"&noBack="+noBack, 'feeframe', false, "POST");
			}
			else
			{
				window._FEE_INFOS = "&TRADE_TYPE_CODE=tradeType&param="+encodeURIComponent(Object.toJSON(params))+"&fee="+encodeURI(encodeURI(Object.toJSON(feeObj)))+"&noBack="+noBack;
				$("feeArea").onshow=redirectTo('personalserv.dealtradefee.DealTradeFee','', '', 'feeframe', false, "POST");
			}
