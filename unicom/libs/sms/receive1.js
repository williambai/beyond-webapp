...
Receiver.prototype.process = function(socket,eventEmitter) {
...
if (process.argv[1] == __filename) {
	...
	var server = require('net').createServer(function(socket) {
		...
		handler.on('report', function(err, command) {
			if (err) return logger.error(err);
			console.log('>> report: ' + JSON.stringify(command));
			sms.report(models, {
				command: command,
			}, function(err, result) {
				if (err) return logger.error(err);
		...
		handler.on('deliver', function(err, command) {
			if (err) return logger.error(err);
			console.log('>> deliver: ' + JSON.stringify(command));
			sms.deliver(models, {
				command: command,
			}, function(err, result) {
				if (err) return logger.error(err);
		...
		socket.on('data', receiver.process(socket,handler));
	});
	server.listen(3000, function() {
		console.log('listen on 8801.');
	});
}