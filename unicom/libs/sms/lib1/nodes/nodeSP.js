var  EE = require('events').EventEmitter
  ...
  , rc = require('../commandCode.js')
  , StreamSpliter = require('../StreamSpliter.js').Class
  // , SuperQueue = require('node-queue-pool')
  , debug = require('debug')
  , logSMGConnect = debug('SGIP:SMGConnect')
  , logSPListen = debug('SGIP:SPListen')
  ;
...
SP.prototype.connect = function(){
  ...
  client.on('timeout', function(){
    logSMGConnect('socket timeout!');
    console.error('connection to SMG socket timeout!');
  });

  logSMGConnect('try to connect to %s:%s', me.SMGHost, me.SMGPort);
  client.connect(me.SMGPort, me.SMGHost, function(){
    logSMGConnect('socket connected');
    ...
    spliter.on('message', function(PDU){
      ...
      if (!bound) {
        ...
        if (res.Result === 0) {
          ...
          logSMGConnect('Bind to smg failed with errCode %d', res.Result);
          console.error('Bind to smg failed with errCode %d', res.Result);
          client.end();
        }
      } else {
...
SP.prototype.listen = function(port, host){
  ...
  var server = net.createServer(function(c){
    logSPListen('socket connected');
    var spliter = new StreamSpliter(c, 'readUInt32BE')
      , bound = false
      ;
    c.on('end', function(){
      logSPListen('socket end');
    });
    c.on('close', function(){
      logSPListen('server close');
    });
    c.on('error', function(){
      logSPListen('socket error');
    });
  ...
  if (host) {
    server.listen(port, host, function(sock){ //'listening' listener
      logSPListen('listen bound at ' + host);
    });
  } else {
    server.listen(port, function(sock){ //'listening' listener
      logSPListen('listen bound');
    });
  }
}
...
if (process.argv[1] === __filename) {
  ...
  switch (lowDebug || sendMethod) {
    ...
    case 2:
      ...
      break;
    case -1:
      option = {'TP_udhi': 1, 'ScheduleTime': util.afterNow(1)};
